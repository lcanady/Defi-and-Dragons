...
contract TimeQuest is Ownable, ReentrancyGuard {
    ...
    function addBonusWindow(bytes32 questId, uint256 startTime, uint256 endTime, uint256 multiplier)
        external
        onlyOwner
    {
        require(startTime >= block.timestamp, "Invalid start time");
        ...
        bonusWindows[questId].push(TimeWindow({ startTime: startTime, endTime: endTime, multiplier: multiplier }));
    ...
    function completeDailyQuest(bytes32 questId, uint256 characterId) external nonReentrant {
        ...
        // Calculate rewards
        uint256 streakBonus = Math.min(progress.currentStreak * quest.streakBonus, quest.maxStreakBonus);
    ...
    function updateSeasonalProgress(bytes32 questId, uint256 characterId, uint256 progress) external {
        require(msg.sender == owner() || approvedTrackers[msg.sender], "Not authorized");

        SeasonalQuest storage quest = seasonalQuests[questId];
        require(quest.active, "Quest not active");
        require(block.timestamp >= quest.startTime && block.timestamp <= quest.endTime, "Not in season");
        ...
        while (
            ...
            emit MilestoneReached(questId, characterId, userProg.milestone, bonus);
    ...
    function applyTimeBonus(bytes32 questId, uint256 baseAmount) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < windows.length; i++) {
            if (
                block.timestamp >= windows[i].startTime && block.timestamp <= windows[i].endTime
