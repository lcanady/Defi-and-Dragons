...
contract AchievementQuest is Ownable, ReentrancyGuard {
    ...
    function recordProgress(
        ...
        if (!achievement.repeatable) {
            ...
            require(
                progress.completions < achievement.maxRepeats || achievement.maxRepeats == 0,
                "Max completions reached"
            );
            require(
                block.timestamp >= progress.lastCompletion + achievement.cooldown,
                "On cooldown"
            );
        }
    ...
    function completeAchievement(bytes32 achievementId, uint256 characterId) internal {
        ...
        uint256 totalReward = achievement.baseReward +
            (achievement.baseReward * achievement.difficulty) / 10;
    ...
    function getCharacterAchievements(uint256 characterId)
        external
        view
        ...
        )
    {
        ...
        nextMilestone = progress.milestone < achievement.milestones.length
            ? achievement.milestones[progress.milestone]
            : 0;
